...
- [ ] 0.1 讀 `app/pages/booking/index.vue` 既有 4 步驟 form 架構
- [ ] 0.2 讀 `i18n/locales/zh.js` 既有 booking key 命名規則（如 `booking.form.* / booking.confirm.*`）
- [ ] 0.3 grep `contactPhone\|flightNumber\|terminal\|notes` 確認沒有意外被佔用

## Stage 1：server 端

- [ ] 1.1 修 `server/routes/nuxt-api/orders/index.post.ts`：
...
- [ ] 4.1 修 `i18n/locales/zh.js`、`en.js`、`ja.js`（三檔結構必須對齊）：
  ```
...
- [ ] 5.1 寫 `docs/decision-log.md` P20 條目（背景 + 強制規範：「booking 表單欄位異動三語言對齊 + Firestore schema 新欄位 nullable 容錯」）
- [ ] 5.2 docs/tasks.md v3.11 標 P20 完成
- [ ] 5.3 一次性 commit + push（fast-forward）

## Stage Gate

- [ ] G1 lint
- [ ] G2 booking 表單能填 4 欄位 + 提交成功
- [ ] G3 必填驗證：contactPhone 空 → 錯誤訊息阻擋送出
- [ ] G4 格式驗證：contactPhone 不是 09 開頭 / 非 10 碼 → 錯誤訊息
- [ ] G5 條件顯示：orderType 切到 charter/transfer 時 flightNumber/terminal 隱藏
- [ ] G6 driver/trip modal 看到新訂單顯示 contactPhone（不再「請透過 LINE 聯絡」）+ flightNumber + terminal + notes（如有）
- [ ] G7 既有訂單（無這些欄位）driver 端開 modal 仍正常顯示，4 欄位區塊安靜隱藏或顯示「—」
- [ ] G8 三語切換 booking 表單 label 正確
