...
- [x] **P3.6-FU2** `line_login_states` 過期清理（2026-08-15）：`purgeExpiredLoginStates` 批次刪過期 doc（consume 成功即刪，但登入未走完的殘留 doc 會無上限累積）；**併進既有 `cleanup-error-logs` 每日 cron 呼叫、不新增 vercel.json cron 條目**（Vercel Hobby 上限 2 個 cron，已滿）；冪等 + 單次批次上限 + 隔日續清（+2 unit tests）。**認證根治全數真正閉環。**
