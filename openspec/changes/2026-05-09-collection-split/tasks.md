...
- [✅] 1.3 lint 通過
- [ ] 1.4 自我驗證：寫一個臨時 console.log 在 admin endpoint，dev 跑一次 `/admin/orders` 確認 `auth.level` 有值（驗證後刪除 log）→ **跳過**：admins doc 在 Stage 10 才人工建，當前 dev 跑只會印 `level=undefined`（expected）；改在 production 部署 + Stage 10 migration 後 e2e 驗證
...
- [ ] 7.4 修 `app/components/common/CommonHeaderUser.vue`（可選）：admin 用戶顯示 level 徽章 → **暫不做**：spec 標 optional，admin/settings 內已有 level 徽章；Header 徽章為純視覺增強，留待後續迭代
- [✅] 7.5 lint 通過
