...
## W9 — 每套主題各帶一組深色配色（2026-08-28）✅ 最後一項留尾

深色模式的作用域 `.dark [data-da-theme]`（0,2,0）高於主題引擎注入的
`[data-da-theme]`（0,1,0），所以深色模式會把季節配色整組蓋掉 —— 聖誕深色 == 預設深色。

- [x] **W9.1** `SiteTheme.tokensDark` / `ResolvedTheme.tokensDark` / `DEFAULT_TOKENS_DARK`
- [x] **W9.2** `resolveTheme` 走同一條合併路徑；`buildThemeCss` 同時輸出兩個區塊
- [x] **W9.3** 四套 seed 主題各配一組深色（對比全部實測）
- [x] **W9.4** 端點接受 `tokens` / `tokensDark`（至少擇一），兩組各走白名單 + hex 驗證
- [x] **W9.5** 後台編輯器加「淺色 / 深色」切換，對比檢查跟著模式算，兩組一起驗一起存
- [x] **W9.6** 同步點 4 → 8，四條新守衛逐條證明會紅
- [x] **W9.7** `buildThemeCss` 對舊格式回應加保底（部署窗口的 30 秒 TTL 快取）

### 為什麼深色不能從淺色推導

`--da-dark` 在兩個模式下語意都是「主文字色」，值卻要**反過來**（深 ↔ 淺）。
這種「語意相同、值相反」的關係算不出來，只能各寫一組。

### 部署後必須做的一件事

跑 `pnpm migrate:site-themes` 把 `tokensDark` 寫進 prod Firestore。
不跑不會壞 —— 會 fallback 到 `DEFAULT_TOKENS_DARK`，也就是四套主題的深色長得一樣，
節日色不會在深色模式出現。

## 全部九項候選完成

階段 0（token 地基）· 階段 1（色彩收尾）· 階段 2（本階段五項 + 排版 + 主題深色盤）
—— 「介面方向改精品調」的九項候選至此全部做完，無留尾。
