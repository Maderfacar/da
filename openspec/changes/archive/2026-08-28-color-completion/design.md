...
## D13 — 語意色的三個「色相分類會犯的錯」，以及怎麼抓出來

P2 用色相把 684 處狀態色分成四族。這個做法對絕大多數點是對的，但會系統性地犯三類錯，
每一類都需要**針對性的檢查**才抓得到 —— 逐張看截圖抓不到，因為它們看起來都「有顏色」。

### 錯誤一：把主色的階當成語意色

`#b8730a` 是**舊琥珀的深階**（實心主色按鈕的 hover），hsl 36°，被分進 `wait`。
同族的還有 `#f7b96a`（亮階）、`#b56a08` / `#b54708`。共 26 處。

抓法：拿 git diff 逐行比對「舊行的字面值」與「新行的 token」，
凡舊值屬於舊色票家族卻被換成語意 token 的，一律改回主色的階。

教訓：**色相分不出「主色的深淺階」與「暖色系語意」**。凡是換過色票的專案，
舊色票的所有衍生階都得先列成排除名單，否則它們會被下一輪語意化吸收掉。

### 錯誤二：把類別色盤壓扁

`is-zh_tw #b91c1c` / `is-en #4338ca` / `is-ja #be185d` 是**語言 chip 的類別色**，
不是狀態。深紅與洋紅同屬紅族，於是 zh_tw 與 ja 一起變成 `--stop` ——
兩個並排的 chip 變同色，那是功能性退步，不是風格調整。

抓法：以 12 行為窗掃描，找「同窗內同一個語意 token 出現 ≥2 次、
但對應的原始色 ≥2 種」的位置。實跑抓出 6 個候選，其中 2 個是真的。

處置：改用三個明顯不同色相的語意 token 撐開（紅 / 藍 / 綠）。
色只是輔助，chip 上本來就有語言文字。

### 錯誤三：把第三方 UI 的擬真色語意化

`#d4e7c5` 是 admin 訊息預覽裡**模仿 LINE 聊天氣泡**的底色。它的目的是「長得像 LINE」，
被換成 `--good` 之後，深色作用域下會變成亮綠，預覽就不像 LINE 了。

處置：新增 `--line-green` / `--line-bubble` 兩個「第三方擬真」token，並明載
**換色票時刻意不跟著換**。列成 token 而不是留字面值的理由是：
未來「元件不得硬編色碼」的守衛才不會一直誤報，最後被迫維護一份例外清單 —— 那比列 token 糟。

### 另外兩個順手修掉的對比問題

- **深色面上的填色標章**：`CommonDrawer` 與 `layouts/driver.vue` 的通知標章原本是
  `#ee5151` / `#f87171` 配白字（3.19:1 / 2.5:1，本來就不及格）。深色作用域讓 `--stop`
  解析成亮階後掉到 1.7:1。改成亮紅底配縞黑字 —— 文字 6.89:1，且標章本身對深底更跳。
- **深色 layout 裡的白底輸入框**：admin 的 diagnostics 篩選器與 bot 文案輸入框是白底，
  但它們在 `[data-surface='dark']` 之內，`:focus` 邊框會拿到深底用的亮銅，
  在白底上只剩 1.9:1（WCAG 2.2 焦點指示需 3:1）。加 `[data-surface='light']` 逃生門，
  標在那幾個元素上把主色拉回 base 階。計費沙盒整頁是淺色，也標在根節點。
