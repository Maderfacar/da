...
- **待修正（broken）**：敘述與專案狀況不符（錯誤工具名、不存在的元件、版本不一致等），MUST 在本次審計中修正。
...
- **THEN** 本次 change 必須修正該檔案，使其降級為 active 或 pending。
...
本次 change MUST 對每個 skill 執行 1 組觸發測試（以 Skill 工具實際呼叫），並將「PASS / 問題描述」寫入 `openspec/changes/audit-project-skills/validation.md`。

#### Scenario: 七個 skill 皆有紀錄

- **WHEN** 檢視 `openspec/changes/audit-project-skills/validation.md`
- **THEN** 檔案中必須包含 `nuxt-frontend`、`nuxt-backend`、`prisma-database`、`element-plus-ui`、`debugging`、`testing`、`deployment` 七個 skill 的觸發結果。
...
本次 change 僅 SHALL 修改 `.claude/skills/` 與 `openspec/` 目錄下的檔案，不得變更 `app/`、`server/`、`package.json`、`Dockerfile`、`CLAUDE.md` 等執行環境內容。
