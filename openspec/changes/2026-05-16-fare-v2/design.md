...
## Brain AI 拍板（2026-05-16）

| # | 問題 | 決策 |
|---|---|---|
| 1 | 資料 host 方式 | **C** — 預處理後精簡壓到 5MB 以下進 git repo `shared/geo/` |
| 2 | OSM 索引更新頻率 | **A** — 一次性產出，永不自動更新；國道有新建/廢線再手動 rebuild |
| 3 | 17 個預設參數值 | **照單全收** — 全部採用 design.md schema 預設值，admin 後台可即時調整 |
| 4 | OSM 過濾範圍 | **A** — 只留 `motorway` + `trunk`，檔案最小（~5MB 內），第二期再擴 `primary/secondary` |
