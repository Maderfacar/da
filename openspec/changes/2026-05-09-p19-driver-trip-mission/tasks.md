...
- [✅] 0.1 讀 P18 decision-log 全部 + P18 spec
- [✅] 0.2 讀 driver/trip / driver/pending / driver/dashboard / driver layout / war-room 現況
- [✅] 0.3 讀 orders/[orderId].patch.ts 現有 status 機 + drivers/available.get.ts + drivers/[id]/location.put.ts
- [✅] 0.4 **Brain AI 確認 design.md 決策 1（drivers.status offline 由 lastActiveAt 推導）** — 等回覆

## Stage 1：訂單狀態擴充（後端）

- [✅] 1.1 修 `server/routes/nuxt-api/orders/[orderId].patch.ts`：
...
- [✅] 2.1 新建 `server/routes/nuxt-api/orders/assigned.get.ts`：
...
- [✅] 3.1 修 `server/routes/nuxt-api/drivers/available.get.ts`：
...
- [✅] 3.2 修 `server/routes/nuxt-api/drivers/[id]/location.put.ts`：
...
- [✅] 4.1 新建 `app/composables/use-driver-geolocation.ts`：
...
- [✅] 6.1 重寫 `app/pages/driver/trip/index.vue`：
...
- [✅] 7.1 新增 `app/protocol/fetch-api/api/order/index.ts` `GetAssignedOrders` API 定義 + type
- [✅] 7.2 修改 `app/protocol/fetch-api/api/order/type.d.ts`：
  - PatchOrderParams.orderStatus 加 `'en_route' | 'arrived_pickup'`
  - 新增 `AssignedOrder` interface
- [✅] 7.3 修改 `app/protocol/fetch-api/api/driver/index.ts` 或對應檔：`UpdateDriverLocation` body 加 `accuracy?: number`
- [✅] 7.4 修改 `i18n/locales/{zh,en,ja}.js`（三檔）：
...
- [✅] 7.5 修改 passenger upcoming 頁 TripStatus type（如 STATUS_TAB_KEYS / STATUS_CLS / STATUS_LABEL 觸及的）：
...
- [✅] 8.1 修改 `app/pages/admin/war-room/index.vue`：
...
- [✅] 8.2 對應 type 更新：`DriverInfo` 加 `accuracy?: number | null` `lastActiveAt: number` `activeOrder?: { orderId, orderStatus } | null`
- [✅] 8.3 lint 通過

## Stage 9：firestore.rules 確認

- [✅] 9.1 確認 `firestore.rules` orders rules 對 driver 改 status 的支援（理論上 server 走 admin SDK bypass rules，rules 不需動；但仍 verify orders rules 沒擋住 owner cancelled / driver patch）

## Stage 10：文件 + commit

- [✅] 10.1 寫 `docs/decision-log.md` P19 條目（背景 + 決策摘要 + 強制規範：「狀態值修改 client/server/i18n 三處對齊」「driver status 不寫 offline 由 lastActiveAt 推導」「watchPosition 在 layout 啟動 vs page 啟動」）
- [✅] 10.2 更新 `docs/tasks.md` v3.10 標 P19 完成
- [✅] 10.3 一次性 commit + push 到 main（fast-forward）

## Stage Gate（部署完成後驗證）

- [✅] G1 lint ✅
- [✅] G2 driver 進 driver 端任一頁 → 彈授權框 → 通過 → 自動開始 watchPosition
- [✅] G3 driver 拒絕授權 → modal 出現 → 重試一次再拒 → 強制踢回 /home
- [✅] G4 admin 指派訂單 → driver/trip 列表出現該訂單（30s polling 內）
- [✅] G5 driver 按「前往上車點」→ status → en_route；war-room 對應 marker activeOrder 也更新顯示
- [✅] G6 五階段全跑：confirmed → en_route → arrived_pickup → in_transit → completed
- [✅] G7 完成後 driver/dashboard `tripsToday++`、`earningsToday += fare`（P18 統計累加 + P19 嚴格狀態機驗證）
- [✅] G8 司機完成最後一張訂單後 → drivers.status 自動切 'online'（war-room 看到綠色而非琥珀）
- [✅] G9 司機關閉 LIFF → 600 秒（10 分鐘）後 war-room 顯示為 offline（灰色 / 從 default filter 隱藏）
- [✅] G10 war-room filter「上線/任務中/離線/全部」切換正確
- [✅] G11 滲透測試：driver A 嘗試 patch driver B 的訂單 status → 預期 403
- [✅] G12 滲透測試：driver patch order status confirmed → completed（跳階段）→ 預期 400/403
