...
**指令**：「spec 預設」— 8 個 Q 全用推 spec 預設。詳見 [design.md §11](design.md#11-決策紀錄)。

| Q | 拍板 | 摘要 |
|---|---|---|
| Q1 | 1a | Richmenu single-active + multi-draft / archived |
| Q2 | 2a | admin 上傳成品圖（不做 grid builder） |
| Q3 | 3a | area action 三選：uri / message / postback |
| Q4 | 4a | template registry server hard-coded |
| Q5 | 5b | A1 endpoint alias 反向相容 + 通用 endpoint 並存 |
| Q6 | 6b | Phase 1 含 5 個 order template（A1 + P39 一次做完） |
| Q7 | 7a | CTA action 與 Q3 對齊 |
| Q8 | 8b | 新獨立頁 `/admin/line-management` 4 tab |

**Q6=6b 連動影響**：原 Phase 5 內 Bot Replies + 公告整合移除（→ P40 留尾），純剩 Diagnostics（可選 / 可延後）。
