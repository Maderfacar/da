# Nuxt Platform Version

## Purpose

定義專案 Nuxt 框架與相關生態套件的版本管理基線：Nuxt 核心、Vue 生態、Nuxt 官方模組、UI 函式庫（Element Plus）、狀態管理（Pinia）、動畫（VueUse Motion）的相容版本策略、升級驗收門檻、與 `nuxt.config.ts` 行為相容契約。
