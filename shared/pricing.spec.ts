...
function metrics(over: Partial<RouteMetrics> = {}): RouteMetrics {
  // 預設「全程高速」（highwayKm=distanceKm, surfaceKm=0）→ 平面加成不觸發
  // → 既有 calculateFareV2 case 不需改 expectation。
  // 要測平面加成的 case 明確覆寫 highwayKm / surfaceKm。
  const distanceKm = over.distanceKm ?? 10;
  return {
    distanceKm,
...
describe('orderType 行程過濾', () => {
  ...
  it('優惠時段套用行程過濾（視窗 1：顛峰塞車費已砍，僅保留 promo / surcharge）', () => {
...
describe('calculateFareV2 — 代表情境', () => {
  it('短程：里程費 < 起跳費 → 起跳費 floor 生效（chargedDistanceFee = baseFare）', () => {
    // distanceKm=10 < surfaceSurcharge.minTotalKm 20 → 不觸發平面加成
    ...
    expect(b.surfaceSurcharge).toBe(0);
  ...
  it('視窗 1：顛峰塞車費已砍 — 同 pickupTime 不再加 jamFee（純里程）', () => {
    // 視窗 1 移除 computeJamFee 後，原本「顛峰加成」場景退化為純里程結果
    // pureJamMinutes 仍存於 metrics 但不影響車資
    ...
    // variableSubtotal = chargedDistanceFee（jamFee 已砍）；475×1 = 475 → 進位 → 500
    expect(b.variableSubtotal).toBe(475);
    expect(b.raw).toBe(475);
...
describe('calculateSurfaceSurcharge', () => {
  const RULE: SurfaceSurchargeRule = DEFAULT_FARE_RULES.surfaceSurcharge;

  it('桃機→內湖特斯拉模擬（totalKm 45 / highway 42 / surface 3）→ surface < surfaceFreeKm 5 → 0', () => {
    expect(calculateSurfaceSurcharge(42, 3, 45, RULE)).toBe(0);
  });

  it('桃機→三芝模擬（totalKm 45 / highway 25 / surface 20）→ (20−5)×30 = 450', () => {
    expect(calculateSurfaceSurcharge(25, 20, 45, RULE)).toBe(450);
  });

  it('桃機→淡水模擬（totalKm 35 / highway 22 / surface 13）→ (13−5)×30 = 240', () => {
    expect(calculateSurfaceSurcharge(22, 13, 35, RULE)).toBe(240);
  });

  it('市區短單（totalKm 8 / highway 0 / surface 8）→ totalKm < minTotalKm 20 → 0', () => {
    expect(calculateSurfaceSurcharge(0, 8, 8, RULE)).toBe(0);
  });

  it('surchargeCap 觸發（totalKm 100 / surface 60）→ min((60−5)×30, 1500) = min(1650, 1500) = 1500', () => {
    expect(calculateSurfaceSurcharge(40, 60, 100, RULE)).toBe(1500);
  });

  it('vehicle override（mpv-vip 設 50/km）→ 採用車型專屬 rate', () => {
    // (20−5)×50 = 750
    expect(calculateSurfaceSurcharge(25, 20, 45, RULE, 50)).toBe(750);
  });

  it('enabled = false → 0', () => {
    expect(calculateSurfaceSurcharge(25, 20, 45, { ...RULE, enabled: false })).toBe(0);
  });

  it('vehicleSurfaceRatePerKm 為 0 / 負數 / NaN → fallback 全域 rate', () => {
    expect(calculateSurfaceSurcharge(25, 20, 45, RULE, 0)).toBe(450);
    expect(calculateSurfaceSurcharge(25, 20, 45, RULE, -10)).toBe(450);
    expect(calculateSurfaceSurcharge(25, 20, 45, RULE, NaN)).toBe(450);
  });

  it('surface 剛好等於 surfaceFreeKm（邊界）→ 0', () => {
    expect(calculateSurfaceSurcharge(20, 5, 25, RULE)).toBe(0);
  });

  it('surface 略大於 surfaceFreeKm（+0.5km）→ 15', () => {
    expect(calculateSurfaceSurcharge(20, 5.5, 25.5, RULE)).toBe(15);
  });
});

describe('calculateFareV2 — 平面道路加成接入', () => {
  it('桃機→三芝（distance 45 / surface 20）→ surfaceSurcharge 450 加在 mountainScaled 後', () => {
    // SEDAN: baseFare=300 / perKmRate=25
    // distanceFee = 10×25 + 20×25×0.9 + 15×25×0.8 = 250 + 450 + 300 = 1000
    // chargedDistanceFee = max(300, 1000) = 1000
    // mountainMul=1; variableScaled=1000
    // surfaceSurcharge = (20−5)×30 = 450
    // raw = 1000 + 0 + 0 + 450 = 1450 → 進位 50 → 1450
    const m = metrics({ distanceKm: 45, highwayKm: 25, surfaceKm: 20 });
    const b = calculateFareV2(SEDAN, m, OFFPEAK_MON_1200, [], RULES);
    expect(b.surfaceSurcharge).toBe(450);
    expect(b.highwayKm).toBe(25);
    expect(b.surfaceKm).toBe(20);
    expect(b.chargedDistanceFee).toBe(1000);
    expect(b.variableScaled).toBe(1000);
    expect(b.raw).toBe(1450);
    expect(b.final).toBe(1450);
  });

  it('桃機→內湖（distance 45 / surface 3）→ surfaceSurcharge 0（不超 surfaceFreeKm）', () => {
    const m = metrics({ distanceKm: 45, highwayKm: 42, surfaceKm: 3 });
    const b = calculateFareV2(SEDAN, m, OFFPEAK_MON_1200, [], RULES);
    expect(b.surfaceSurcharge).toBe(0);
    // distanceFee = 10×25 + 20×25×0.9 + 15×25×0.8 = 1000；raw = 1000；final = 1000
    expect(b.final).toBe(1000);
  });

  it('vehicle.surfaceRatePerKm 覆寫全域 rate', () => {
    const mpvVip = { baseFare: 800, perKmRate: 60, surfaceRatePerKm: 50 };
    // distanceFee = 10×60 + 20×60×0.9 + 15×60×0.8 = 600 + 1080 + 720 = 2400；chargedDistanceFee = max(800, 2400) = 2400
    // surfaceSurcharge = (20−5)×50 = 750
    // raw = 2400 + 750 = 3150 → 進位 50 → 3150
    const m = metrics({ distanceKm: 45, highwayKm: 25, surfaceKm: 20 });
    const b = calculateFareV2(mpvVip, m, OFFPEAK_MON_1200, [], RULES);
    expect(b.surfaceSurcharge).toBe(750);
    expect(b.final).toBe(3150);
  });

  it('surfaceSurcharge 不被山區係數放大', () => {
    // 山區 3 訊號達標 + surface 20km
    // distanceFee = 10×25 + 20×25×0.9 + 15×25×0.8 = 1000
    // mountainMul = 1.5；variableScaled = 1500
    // surfaceSurcharge = 450（不乘 1.5）
    // raw = 1500 + 450 = 1950 → 進位 → 1950
    const m = metrics({
      distanceKm: 45,
      highwayKm: 25,
      surfaceKm: 20,
    ...
    const b = calculateFareV2(SEDAN, m, OFFPEAK_MON_1200, [], RULES);
    expect(b.mountainMul).toBe(1.5);
    expect(b.variableScaled).toBe(1500);
    expect(b.surfaceSurcharge).toBe(450);
    expect(b.raw).toBe(1950);
    expect(b.final).toBe(1950);
  });

  it('短單（distance 8 / surface 8）→ totalKm < minTotalKm 20 → 不加成', () => {
    const m = metrics({ distanceKm: 8, highwayKm: 0, surfaceKm: 8 });
    const b = calculateFareV2(SEDAN, m, OFFPEAK_MON_1200, [], RULES);
    expect(b.surfaceSurcharge).toBe(0);
  });

  it('rules.surfaceSurcharge.enabled=false → 加成 0', () => {
    const r: FareRules = { ...RULES, surfaceSurcharge: { ...RULES.surfaceSurcharge, enabled: false } };
    const m = metrics({ distanceKm: 45, highwayKm: 25, surfaceKm: 20 });
    const b = calculateFareV2(SEDAN, m, OFFPEAK_MON_1200, [], r);
    expect(b.surfaceSurcharge).toBe(0);
  });
});
...
function charterMetrics(over: Partial<RouteMetrics> = {}): RouteMetrics {
  // 預設全程高速；charter 引擎本身不算 surfaceSurcharge，但 metrics 仍需 highwayKm/surfaceKm 欄位
  const distanceKm = over.distanceKm ?? 50;
  return {
    distanceKm,
