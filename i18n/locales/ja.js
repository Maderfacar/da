export default {
  ...
  fare: {
    ...
    engine: {
      ...
      card2Body: '高低差、カーブの密度、リアルタイム速度、通過する県市、高速道路の距離 — これらをまとめて数式に投入します。山道・越県・ピーク時間・通行料は、シグナルに応じて自動適用されます。',
    ...
    calc: {
      label: 'TRY IT',
      title: '試してみる',
      desc: '大まかな距離と条件を入力すると、数式がどのような数字を返すかご覧いただけます。実際の予約時には、完全なシグナルで再計算します。',
      field: {
        distance: 'ルート距離（km）',
        vehicle: '車種',
        pickup: '乗車時刻',
      ...
      advanced: {
        toggleClosed: '▶ 詳細オプション（山間部 / 越県 / 高速）',
        toggleOpen: '▼ 詳細オプション（山間部 / 越県 / 高速）',
        mountain: '山間部ルート',
        crossCounty: '越県',
        crossCountyOptions: {
          0: 'なし',
        ...
        tpeMetro: '全行程が台北・新北・桃園のみ',
        freeway: '高速道路距離（km）',
        freewayHelper: '高速を使わない場合は 0',
      },
      btn: {
        calculate: '料金を計算',
        reset: 'リセット',
      ...
      error: {
        distance: '距離は 0 以上の数字を入力してください',
        freeway: '高速距離は 0 以上の数字を入力してください',
        vehicle: '車種を選択してください',
      ...
      result: {
        ...
        surcharge: '時間帯割増',
        promo: '優待割引',
        final: '最終料金（{round} 元単位で切り上げ）',
      },
      disclaimer: 'この試算は目安です。リアルタイム交通の渋滞費は含まれていません。ご予約時には Google ルートと現在の交通データで再計算され、ご予約画面の金額が確定額となります。',
    },
    notes: {
      label: 'FARE NOTES',
      title: '料金に含まれるもの',
  ...
  faq: {
    ...
    items: {
      ...
      fareSurcharge: { q: '料金が通常より高くなることがあるのはなぜですか？', a: '料金は基本初乗り料金、距離料金、および以下の追加項目で構成されており、すべての内訳はご予約時にリアルタイムで表示され、隠れた料金はありません：① 山間部係数（標高差・カーブ度・低速区間などが大きいルートで自動適用）；② 県境跨ぎ加算（県を跨ぐ回数に応じて加算）；③ 高速道路通行料（高速距離に応じて計算）；④ ピーク時混雑料金（平日朝夕の通勤時間帯に純渋滞分単位で計算）；⑤ 深夜・祝祭日・特定時間帯の加算（管理画面の時間帯ルールに基づく）。すべてのルールは管理画面でカスタマーサポートが設定し、ご予約時の見積額にはこれらの加算が既に含まれています。' },
