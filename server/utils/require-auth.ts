/**
 * Server-side 身分驗證 helper（P14 上線安全修復；認證根治 Phase 0 重構）
 *
 * 取身分兩條入口共用同一套解析（resolveIdentity），杜絕雙路徑語意分岔：
 *   1. session cookie（da_session）— 認證根治後的主路徑（verifySessionCookie）
 *   2. Authorization: Bearer <Firebase ID token> — 過渡期 fallback（既有行為，100% 等價）
 *
 * 用法：
...
import { verifySessionCookie, SESSION_COOKIE_NAME } from '@@/utils/session-cookie';
...
/**
 * 共用身分解析：拿到 decoded（不論來自 session cookie 或 Bearer idToken）→ 解析完整身分。
 * roles/approved Firestore 即時 SSOT + admin level/permissions + admin 2FA gate。
 * cookie 與 Bearer 兩條入口共用此函式，確保語意 100% 一致。
 */
export async function resolveIdentity(event: H3Event, decoded: DecodedIdToken): Promise<AuthResult> {
  ...
  if (roles.includes('admin')) {
    try {
      ...
      console.error('[resolveIdentity] admins doc read failed:', err);
    }
  }
...
export async function getAuthFromEvent(event: H3Event): Promise<AuthResult> {
  // 認證根治：先 session cookie（主路徑），無 cookie 時退回 Authorization: Bearer（過渡相容）。
  // 無 cookie 分支與導入本變更前的 Bearer 行為 100% 等價。
