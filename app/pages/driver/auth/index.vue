...
const { isSignIn, roles, authResolved, liffReady } = storeToRefs(authStore);
...
async function ClickLineLogin() {
  if (!liffReady.value) return;
  liffLoading.value = true;
  try {
    const liff = (await import('@line/liff')).default;
    liff.login(); // liff.init() 已由 InitAuthFlow 完成
  } catch {
...
.PageDriverAuth
  ...
  .PageDriverAuth__card
    ...
    button.PageDriverAuth__line-btn(
      ...
      :disabled="liffLoading || !liffReady"
    )
